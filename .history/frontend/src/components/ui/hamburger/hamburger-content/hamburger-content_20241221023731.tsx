  ...
  return (
    ...
    `} */>
      <div className={styles['hamburger-content__block']}>
        ...
        <ul className={styles['hamburger-content__block-list']}>
          <Link href='/' className={styles['logo-link_mobile']}>
            <LogoImage />
          </Link>
          {navItems.map((item, index) => (
            <li key={index} className={styles['hamburger-content__block-item']}>
              <Link
                href={item.href}
                onClick={(e) => {
                ...
                className={`${styles['hamburger-content__block-link']} ${styles.link}`}
              >
                <div className={styles.link__inner}>
                  {item.title}
                  {item.subItems && item.subItems.length > 0 && (
                    <IoIosArrowDown
                      className={styles['hamburger-content__link-arrow']}
                    />
                  )}
                </div>
                {activeDropdownSecond === index && item.subItems && item.subItems.length > 0 && (
                  <div className={styles['hamburger-content__dropdown']}>
                    <Divider />
                    <div className={styles['hamburger-content__dropdown-inner']}>
                      <ul>
