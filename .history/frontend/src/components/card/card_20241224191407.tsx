...
export default function Card({ product, info = true }: ICard) {
  ...
  useEffect(() => { 
    const handleResize = () => { 
      ...
      if (isNowMobile) { setQuickViewModal(false); setSelectSizeModal(false); } }; handleResize(); window.addEventListener('resize', handleResize); return () => { window.removeEventListener('resize', handleResize); }; }, []);
