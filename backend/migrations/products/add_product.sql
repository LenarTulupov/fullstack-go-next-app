-- Уникальное ограничение на название продукта и цвет, если оно еще не добавлено
...
-- Вставка продукта с проверкой на существование продукта с таким же названием и цветом
...
-- Вставка миниатюры продукта, если она еще не добавлена
...
-- Вставка изображений для продукта с проверкой на существование каждого изображения
...
-- Вставка данных в product_sizes с проверкой на существование
...
WITH inserted_product_2 AS (
    ...
    SELECT 
        'SUPER STRETCH TAPERED TAILORED TROUSER',
        'Work clothes don’t have to be boring, and these work trousers are a secure style...',
        20.00,
        25.00,
        0,
        (SELECT id FROM categories WHERE name = 'trousers'),
...
-- Аналогичная вставка миниатюр и изображений
INSERT INTO thumbnail (product_id, color_id, thumbnail, created_at, updated_at)
SELECT 
    (SELECT id FROM products WHERE title = 'SUPER STRETCH TAPERED TAILORED TROUSER' AND color_id = (SELECT id FROM colors WHERE name = 'blue') LIMIT 1), 
...
-- Аналогичная вставка изображений для второго продукта
INSERT INTO images (product_id, color_id, image, created_at, updated_at)
SELECT 
    (SELECT id FROM products WHERE title = 'SUPER STRETCH TAPERED TAILORED TROUSER' LIMIT 1),
    (SELECT id FROM colors WHERE name = 'blue' LIMIT 1),
...
FROM (
    VALUES 
        ('https://media.boohoo.com/i/boohoo/fzz77463_navy_xl/female-navy-super-stretch-tapered-tailored-trouser'),
