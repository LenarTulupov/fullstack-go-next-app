  ...
  let filteredProducts = products
    ...
    .filter(
      (product) =>
        ...
        (!selectedPrice ||
          (parseFloat(product.price_new) >=
            parsePriceRange(selectedPrice).min &&
            parseFloat(product.price_new) <=
              parsePriceRange(selectedPrice).max))
    );

  if (selectedSortOption === "low to high") {
    filteredProducts = filteredProducts.sort(
      (a, b) => parseFloat(a.price_new) - parseFloat(b.price_new)
    );
  } else if (selectedSortOption === "high to low") {
    filteredProducts = filteredProducts.sort(
      (a, b) => parseFloat(b.price_new) - parseFloat(a.price_new)
    );
  }
